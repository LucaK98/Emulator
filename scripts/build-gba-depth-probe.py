#!/usr/bin/env python3
"""Assembles tests/roms/gba-depth-probe.gba.

A small mode-0 cartridge that sets up two background layers, two palettes and
three objects, so the depth decoder can be checked against mGBA's own picture.
Unlike the far-cartridge probe it needs no padding, so this is the plain case:
assemble, prepend a header, done.
...
SOURCE = ROOT / "tests/roms/src/gba-depth-probe.s"
OUTPUT = ROOT / "tests/roms/gba-depth-probe.gba"
...
def main() -> int:
    ...
    rom[0xA0:0xAC] = b"DEPTHPROBE".ljust(12, b"\x00")
    rom[0xAC:0xB0] = b"CDPP"
    rom[0xB0:0xB2] = b"01"
    rom[0xB2] = 0x96
