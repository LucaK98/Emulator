...
**Kartengedächtnis.** Eine Konsole hält kaum mehr Karte im Speicher als sie zeigt — beim Game Boy 32 mal 32 Kacheln, etwa anderthalb Bildschirme — und schreibt die nächste Kachelspalte erst kurz bevor sie ins Bild scrollt. Eine Tiefensicht, die über den Ausschnitt der Konsole hinausreicht, hätte dort also nichts Echtes zu zeigen. Deshalb führt die App eine eigene Kopie: die Kacheln, die gerade sicher aktuell sind, werden in eine große Weltkarte geschrieben, und der Boden wird aus dieser Karte gezeichnet. Was man gesehen hat, bleibt stehen, und der Blick reicht so weit wie das Erkundete.

Der schwierige Teil ist das Vergessen zur richtigen Zeit — beim Betreten eines Hauses wird die ganze Welt ausgetauscht, während der Scroll sich kaum bewegt. Drei Anzeichen setzen zurück: die Kachelgrafik wird großflächig neu geschrieben, der Ausschnitt springt weiter als man laufen kann, oder das Bild widerspricht dem Gemerkten an derselben Stelle.

Das Erkundete bleibt über Sitzungen hinweg erhalten. Es wird zusammen mit dem automatischen Spielstand abgelegt und nur zusammen mit ihm wieder aufgenommen: die gespeicherte Position ist eine mitgezählte Summe und ergibt nur dort einen Sinn, wo das Spiel weitermacht. Passt die Kachelgrafik nicht oder ist der Sprung zu groß, um gelaufen zu sein, wird die Karte verworfen statt an die falsche Stelle gelegt. Im Pausenmenü steht, wie viel Welt gerade vorliegt.
